...
_BASELINE_ROOT = Path(__file__).resolve().parents[1] / "baseline_models"
_K_GNN_ROOT = _BASELINE_ROOT / "k-gnn"
_LEGACY_K_GNN_ROOT = Path(__file__).resolve().parents[1] / "k-gnn"
...
def _load_k_gnn_components():
    ...
    try:
        from k_gnn import ConnectedThreeMalkin, GraphConv, TwoMalkin, ThreeMalkin, avg_pool
    ...
    return Data, GraphConv, TwoMalkin, ThreeMalkin, ConnectedThreeMalkin, avg_pool
...
class _SourceKGNNAdapter(nn.Module):
    ...
    def __init__(self, in_concepts, in_roles, out_concepts, out_roles, hidden_size, num_layers, max_objects=None):
        super().__init__()
        Data, GraphConv, TwoMalkin, ThreeMalkin, _, avg_pool = _load_k_gnn_components()
        self._Data = Data
    ...
    def forward(self, concepts, roles):
        ...
        for concepts_one, roles_one in zip(concepts, roles):
            ...
            for convolution in self.tuple_convs:
                tuple_features = torch.relu(
                    convolution(tuple_features, data.edge_index_2 if self.tuple_order == 2 else data.edge_index_3)
                )

            concept_logits = self.node_head(node_features).transpose(0, 1)
        ...
        return torch.stack([output[0] for output in outputs]), torch.stack(
